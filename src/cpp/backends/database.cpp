...
std::vector<int> Database::getQuestionCount() const {
    ...
    if (duckdb_query(conn_,
        ...
        for (idx_t i = 0; i < rows; i++)
            resultVec.push_back(duckdb_value_int32(&result, 1, i));
    }
    duckdb_destroy_result(&result);
    return resultVec;
}

std::vector<QuestionData> Database::getQuestions(const int gamemode, const int count) const {
